...
pub(crate) fn insert_path_and_string_options(
    ...
    let to_relative = |val: &str| -> String {
        ...
        if !tsox_core::tspath::path_is_absolute(val) {
            return val.to_string();
        }

        let abs_val = tsox_core::tspath::get_normalized_absolute_path(val, "");
        let abs_config_dir = tsox_core::tspath::get_normalized_absolute_path(&config_dir, "");
        let abs_config_dir_with_sep =
            tsox_core::tspath::ensure_trailing_directory_separator(&abs_config_dir);
...
pub(crate) fn insert_list_options(
    ...
    if let Some(paths) = &options.paths {
        let mut paths_map = tsox_core::json::Map::new();
        for (k, v) in paths {
            paths_map.insert(
                k.clone(),
