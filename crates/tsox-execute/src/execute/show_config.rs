...
pub(crate) fn show_config(sys: &dyn System, config: &ParsedCommandLine) {
    ...
    let mut map = tsox_core::json::Map::new();
    ...
    let mut top = tsox_core::json::Map::new();
    if !map.is_empty() {
    ...
    if !config.references.is_empty() {
        let refs: Vec<Value> = config
            ...
            .iter()
            .map(|r| {
                let mut obj = tsox_core::json::Map::new();
    ...
    let _ = tsox_core::json::marshal_indent_write(&mut writer, &json, "    ");
