...
pub(crate) fn tsc_compilation(
    ...
    if !options.project.is_empty() {
        ...
        let file_or_directory = tsox_core::tspath::normalize_path(
            &tsox_core::tspath::combine_paths(sys.current_directory(), &[&options.project]),
        );
        if sys.fs().directory_exists(&file_or_directory) {
            config_file_name =
                tsox_core::tspath::combine_paths(&file_or_directory, &["tsconfig.json"]);
        ...
        let search_path = tsox_core::tspath::normalize_path(sys.current_directory());
        config_file_name =
