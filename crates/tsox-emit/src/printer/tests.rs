use super::*;
use tsox_checker::binder::Binder;
use tsox_frontend::ast::node_data_generated::IdentifierData;
...
fn generated_name_for_identifier_1() {
    ...
    let tsox_frontend::ast::node_data_generated::NodeData::SourceFile(d) = stmt else {
...
fn generated_name_for_identifier_2() {
    ...
    let factory = NodeFactory::new(&ec);
    let (file, _) = parse_and_bind("function f() {}");
    let tsox_frontend::ast::node_data_generated::NodeData::SourceFile(d) = &file.node.data else {
        panic!("expected SourceFile");
    };
    let func = &d.statements.nodes[0];
    let n = func.name().unwrap();
...
fn generated_name_for_namespace_3() {
    ...
    let tsox_frontend::ast::node_data_generated::NodeData::ModuleDeclaration(ns1_data) =
        &ns1_outer.data
    else {
    ...
    let tsox_frontend::ast::node_data_generated::NodeData::ModuleDeclaration(ns2_data) =
        &ns2_outer.data
    else {
        panic!("expected ModuleDeclaration");
    };
...
fn generated_name_for_method_1() {
    ...
    let tsox_frontend::ast::node_data_generated::NodeData::ClassDeclaration(class_data) =
        &class_node.data
    else {
