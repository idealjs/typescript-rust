...
pub fn emit_source_file_with_common_dir(
    ...
    if source_file.script_kind == tsox_frontend::ast::ScriptKind::Js
...
pub(crate) fn emit_js_with_sourcemap(
    ...
    let source_root = if options.source_root.is_empty() {
        ...
        tsox_core::tspath::ensure_trailing_directory_separator(&options.source_root)
    };

    let sources_dir = tsox_core::tspath::get_directory_path(js_path);
...
pub(crate) fn get_js_output_path(
    ...
    if !options.out_dir.is_empty() {
        ...
        let without_ext = tsox_core::tspath::remove_file_extension(&path_in_new_dir);
        format!("{without_ext}{extension}")
    } else {
        let without_ext = tsox_core::tspath::remove_file_extension(file_name);
        format!("{without_ext}{extension}")
    }
}
