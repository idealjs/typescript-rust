...
pub(crate) fn compute_common_source_directory(options: &CompilerOptions) -> String {
    ...
    tsox_core::tspath::ensure_trailing_directory_separator(&common_dir)
}
...
pub(crate) fn get_source_file_path_in_new_dir(
    ...
    if common_source_directory.is_empty() {
        return tsox_core::tspath::combine_paths(
            new_dir_path,
            &[tsox_core::tspath::get_base_file_name(file_name).as_str()],
        );
    }

    let common_with_sep =
        tsox_core::tspath::ensure_trailing_directory_separator(common_source_directory);
    let normalized_file = tsox_core::tspath::normalize_slashes(file_name);
    if let Some(stripped) = normalized_file.strip_prefix(&common_with_sep) {
        return tsox_core::tspath::combine_paths(new_dir_path, &[stripped]);
    }
    ...
    let abs_file = tsox_core::tspath::get_normalized_absolute_path(file_name, "");
    let abs_common = tsox_core::tspath::get_normalized_absolute_path(&common_with_sep, "");
    let abs_common_with_sep = tsox_core::tspath::ensure_trailing_directory_separator(&abs_common);
...
pub(crate) fn get_output_extension(file_name: &str) -> &'static str {
    ...
    if tsox_core::tspath::file_extension_is_one_of(file_name, &[".cts", ".cjs"]) {
        return ".cjs";
    }
    ".js"
}
