...
fn emit_skips_json_files() {
    ...
    sf_with_name.script_kind = tsox_frontend::ast::ScriptKind::Json;
...
fn emit_source_file_skips_js_when_no_emit_for_js_files() {
    ...
    sf.script_kind = tsox_frontend::ast::ScriptKind::Js;
...
fn sourcemap_mappings_decode_to_correct_positions() {
    ...
    let raw: crate::sourcemap::RawSourceMap = tsox_core::json::unmarshal(&map).expect("valid JSON");
...
fn sourcemap_write_file_creates_map() {
    ...
    let result = emit_source_file_with_common_dir(
        &sf,
        &opts,
        &tsox_tsoptions::vfs::InMemoryFS::new(),
        "",
        &|path, content| {
            written
                .borrow_mut()
