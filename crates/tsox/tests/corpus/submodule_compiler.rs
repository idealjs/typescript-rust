...
use tsox_compile::compiler::{CompilerHostImpl, Program, ProgramOptions};
use tsox_core::core::compiler_options::CompilerOptions;
use tsox_frontend::diagnosticwriter::format_diagnostic_compact;
use tsox_tsoptions::tsoptions::apply_test_settings;
use tsox_tsoptions::vfs::InMemoryFS;

use crate::common::baseline::{self, KnownDiffs, NO_CONTENT};
use crate::common::case_parser::{extract_settings, split_units};
...
fn submodule_dir() -> String {
    format!("tests/corpus/testdata/tests/cases/{}", suite())
}
            ...
            .map(|(suffix, config_settings)| {
                let (mut compiler_options, unrecognized) = match &tsconfig {
                    Some((parsed_config, config_path)) => {
                        let (mut opts, unrec) = tsox_tsoptions::tsoptions::apply_test_settings_with_base(
                            &config_settings,
                ...
                if std::env::var_os("TSOX_PROBE_SKIPLIB").is_some() {
                    compiler_options.skip_default_lib_check = tsox_core::core::tristate::Tristate::True;
                }
...
fn detect_tsconfig(
...
) -> Option<(tsox_tsoptions::tsoptions::ParsedCommandLine, String)> {
    ...
    for unit in units {
        ...
        while !parent.is_empty() {
            ...
            let next = tsox_core::tspath::get_directory_path(&parent);
            if next == parent {
                break;
            }
            parent = next;
    ...
    let cwd = if units.iter().any(|u| tsox_core::tspath::is_rooted_disk_path(&u.name)) {
        "/.src"
    } else {
        "/proj"
    };
    ...
    Some((
        tsox_tsoptions::tsoptions::get_parsed_command_line_of_config_file(
            &config_path,
...
fn build_and_check(
    ...
    let rooted: Vec<&crate::common::case_parser::TestUnit> = match &config_roots {
        Some(names) => units
            .iter()
    ...
    let all_rooted = units.iter().any(|u| {
        tsox_core::tspath::is_rooted_disk_path(&u.name)
    });
...
fn render_errors_baseline(diags: &[Diagnostic], program_len: usize, pretty: bool) -> String {
    ...
    if !globals.is_empty() {
        ...
        out.push('\n');
        for d in globals {
            let msg = tsox_frontend::diagnosticwriter::message_text(d, None);
...
fn crate_line_col(d: &Diagnostic) -> (usize, usize) {
    if let Some(file) = &d.file {
        tsox_frontend::diagnosticwriter::line_and_character(&file.line_map, &file.text, d.loc.pos())
    } else {
        (0, 0)
    }
}
