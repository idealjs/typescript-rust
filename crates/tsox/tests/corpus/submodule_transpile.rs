...
use tsox_compile::transpile::{TranspileOptions, transpile_declaration, transpile_module};

use crate::common::baseline::{self, KnownDiffs};
use crate::common::case_parser::{extract_settings, split_units};

const SUBMODULE_DIR: &str = "tests/corpus/testdata/tests/cases/transpile";
...
fn js_output_extension(file_name: &str, jsx: tsox_core::core::compiler_options::JsxEmit) -> &'static str {
    let lower = file_name.to_ascii_lowercase();
    if lower.ends_with(".mts") || lower.ends_with(".mjs") {
        ".mjs"
    } else if lower.ends_with(".cts") || lower.ends_with(".cjs") {
        ".cjs"
    } else if jsx == tsox_core::core::compiler_options::JsxEmit::Preserve
    ...
    let mut keyed: Vec<(String, usize, usize, i32, &Diagnostic)> = diags
        ...
        .map(|d| {
            let (file_name, line, col) = if let Some(f) = &d.file {
                let (l, c) = tsox_frontend::diagnosticwriter::line_and_character(
                    &f.line_map,
                    &f.text,
                    d.loc.pos(),
                );
...
fn run_kind(
    ...
    for unit in units {
        ...
        let output_file_name = tsox_core::tspath::change_extension(&unit.name, output_ext);
...
fn run_case(
    ...
    let worker = Command::new(exe)
        .arg("--exact")
        .arg("submodule_transpile::submodule_transpile_cases")
