    ...
    checker
        ...
        .map(|d| {
            ...
            for c in &d.message_chain {
                s.push('\n');
                s.push_str("  ");
                s.push_str(&tsox_frontend::diagnosticwriter::message_text(c, None));
            }
            s
        })
        .collect()
...
fn optional_member_stays_t_when_strict_null_checks_off() {
    let lines: Vec<Vec<i32>> = [false, true]
        .iter()
        .map(|strict| {
            let diags = crate::convergence_tests::checker_node_format_tests::check_files(
                &[(
                    "entry.ts",
                ...
                "/proj/entry.ts",
                |o| o.strict_null_checks = tsox_core::core::tristate::Tristate::from(*strict),
            );
            diags
        })
        .collect();
...
fn node10_program_reports_deprecation_and_alternate_result() {
    ...
    options.module_resolution = tsox_core::core::compiler_options::ModuleResolutionKind::Node10;
    options.target = tsox_core::core::compiler_options::ScriptTarget::ES2015;
    options.module = tsox_core::core::compiler_options::ModuleKind::CommonJS;
...
fn per_file_jsx_pragma_overrides_option_factory_for_2874() {
    ...
    let diags = crate::convergence_tests::checker_node_format_tests::check_files(
        ...
        "/proj/reacty.tsx",
        |o| {
            o.jsx = tsox_core::core::compiler_options::JsxEmit::React;
            o.jsx_factory = "p".to_string();
            o.module = tsox_core::core::compiler_options::ModuleKind::CommonJS;
            o.target = tsox_core::core::compiler_options::ScriptTarget::ES2015;
        },
    );
    assert!(
