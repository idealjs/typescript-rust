...
impl NameResolver {
        ...
        if let Some(_message) = name_not_found_message {
            if let Some(opts) = &self.compiler_options {
                if opts.get_isolated_modules() {
                    ...
                    self.error(
                        original_location,
                        &tsox_core::diagnostics::messages_generated::CANNOT_ACCESS_0_FROM_ANOTHER_FILE_WITHOUT_QUALIFICATION_WHEN_1_IS_ENABLED_USE_2_INSTEAD,
                        &[
                            name.to_string(),
    ...
    pub(crate) fn resolve_class_or_interface_case(
        ...
        if name_not_found_message.is_some() {
            self.error(
                original_location,
                &tsox_core::diagnostics::messages_generated::STATIC_MEMBERS_CANNOT_REFERENCE_CLASS_TYPE_PARAMETERS,
                &[],
            );
        }
    }
    ...
    pub(crate) fn resolve_computed_property_name_case(
        ...
        if name_not_found_message.is_some() {
            self.error(
                original_location,
                &tsox_core::diagnostics::messages_generated::A_COMPUTED_PROPERTY_NAME_CANNOT_REFERENCE_A_TYPE_PARAMETER_FROM_ITS_CONTAINING_TYPE,
                &[],
            );
        }
        false
