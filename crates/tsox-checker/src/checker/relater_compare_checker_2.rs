...
impl Checker {
    pub(crate) fn elaborate_array_literal(
        ...
        let elements = match &node.data {
            tsox_frontend::ast::NodeData::ArrayLiteralExpression(d) => &d.elements,
            _ => return false,
        };
        let _ = source;
        let mut reported = false;
        for (i, element) in elements.iter().enumerate() {
            if element.kind == tsox_frontend::ast::SyntaxKind::OmittedExpression
                || element.kind == tsox_frontend::ast::SyntaxKind::SpreadElement
            {
                continue;
            }
