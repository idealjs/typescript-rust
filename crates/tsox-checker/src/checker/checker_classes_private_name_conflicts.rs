            ...
            && let tsox_frontend::ast::NodeData::ClassDeclaration(cd) = &cls.data
        {
            ...
            if !my_name.is_empty() && !my_name.starts_with('#') {
                let kind_of = |m: &Arc<Node>| match &m.data {
                    tsox_frontend::ast::NodeData::PropertyDeclaration(_) => "prop",
                    tsox_frontend::ast::NodeData::MethodDeclaration(d) => {
                        if d.body.is_some() {
                            "method-body"
                        } else {
                    ...
                    tsox_frontend::ast::NodeData::GetAccessorDeclaration(_) => "get",
                    tsox_frontend::ast::NodeData::SetAccessorDeclaration(_) => "set",
                    _ => "",
                };
