...
impl Checker {
    ...
    pub(crate) fn syntactic_truthy_semantics(&mut self, node: &Arc<Node>) -> (bool, bool) {
        ...
        loop {
            match &n.data {
                ...
                tsox_frontend::ast::NodeData::NonNullExpression(p) => n = Arc::clone(&p.expression),
                tsox_frontend::ast::NodeData::AsExpression(p) => n = Arc::clone(&p.expression),
                tsox_frontend::ast::NodeData::TypeAssertion(p) => n = Arc::clone(&p.expression),
                _ => break,
            }
        }
        use SyntaxKind::*;
        match n.kind {
            ...
            ConditionalExpression => {
                if let tsox_frontend::ast::NodeData::ConditionalExpression(d) = &n.data {
    ...
    pub(crate) fn check_truthiness_of_type(&mut self, node: &Arc<Node>) {
        ...
        let message = if always && !never {
            ...
            tsox_core::diagnostics::messages_generated::THIS_KIND_OF_EXPRESSION_IS_ALWAYS_FALSY
        } else {
            return;
        };
