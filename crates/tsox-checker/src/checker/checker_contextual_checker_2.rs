    ...
    pub(crate) fn is_valid_const_assertion_argument(&mut self, node: &Arc<Node>) -> bool {
        match node.kind {
            ...
            SyntaxKind::PrefixUnaryExpression => match &node.data {
                tsox_frontend::ast::NodeData::PrefixUnaryExpression(p) => {
    ...
    pub(crate) fn check_delete_operand(&mut self, operand: &Arc<Node>) {
        ...
        while target.kind == SyntaxKind::ParenthesizedExpression {
            let inner = match &target.data {
                tsox_frontend::ast::NodeData::ParenthesizedExpression(p) => &p.expression,
                _ => break,
            };
            target = inner;
        }
        match target.kind {
            ...
            SyntaxKind::PropertyAccessExpression | SyntaxKind::ElementAccessExpression => {
                ...
                if matches!(&target.data, tsox_frontend::ast::NodeData::PropertyAccessExpression(d) if d.name.kind == SyntaxKind::PrivateIdentifier)
                {
                    self.diagnostics.add(tsox_frontend::ast::Diagnostic::new(
                        self.current_file.clone(),
                        target.loc,
    ...
    pub(crate) fn check_const_assignment_target(&mut self, operand: &Arc<Node>) {
        ...
        loop {
            target = match &target.data {
                tsox_frontend::ast::NodeData::ParenthesizedExpression(p) => &p.expression,
                tsox_frontend::ast::NodeData::NonNullExpression(n) => &n.expression,
                _ => break,
            };
        }
        let operand = target;
