...
impl Checker {
    pub(crate) fn check_duplicate_function_implementations(&mut self, node: &Arc<Node>) {
        ...
        let stmts = match &parent.data {
            tsox_frontend::ast::NodeData::SourceFile(sf) => Some(&sf.statements),
            tsox_frontend::ast::NodeData::ModuleBlock(mb) => Some(&mb.statements),
            _ => None,
        };
        ...
        let fns: Vec<&Arc<Node>> = stmts
            ...
            .filter(|s| {
                s.kind == SyntaxKind::FunctionDeclaration
                    && matches!(&s.data, tsox_frontend::ast::NodeData::FunctionDeclaration(d) if d
                        .name
                        .as_ref()
    ...
    pub(crate) fn check_overload_implementation_follows(&mut self, node: &Arc<Node>) {
        ...
        if next.as_ref().is_some_and(|n| {
            matches!(&n.data, tsox_frontend::ast::NodeData::FunctionDeclaration(d) if d
                .name
                .as_ref()
        ...
        if let Some(n) = &next
            && matches!(&n.data, tsox_frontend::ast::NodeData::FunctionDeclaration(d) if d.body.is_some())
            && n.kind == SyntaxKind::FunctionDeclaration
        {
            if let tsox_frontend::ast::NodeData::FunctionDeclaration(d) = &n.data
    ...
    pub(crate) fn check_multiple_constructor_implementations(&mut self, node: &Arc<Node>) {
        ...
        let tsox_frontend::ast::NodeData::ClassDeclaration(cd) = &class.data else {
            return;
        };
        ...
        let bodied = ctors
            .iter()
            .filter(|c| {
                matches!(&c.data, tsox_frontend::ast::NodeData::ConstructorDeclaration(d) if d.body.is_some())
            })
            .count();
        if bodied < 2 {
            return;
