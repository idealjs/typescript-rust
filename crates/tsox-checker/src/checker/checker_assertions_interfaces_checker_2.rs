...
impl Checker {
    ...
    pub(crate) fn check_function_overloads_recursive(&mut self, statements: &[Arc<Node>]) {
        ...
        for s in statements {
            match &s.data {
                tsox_frontend::ast::NodeData::Block(d) => {
                ...
                tsox_frontend::ast::NodeData::ModuleDeclaration(d) => {
                    ...
                    if let Some(body) = &d.body
                        ...
                        && let tsox_frontend::ast::NodeData::Block(bd) = &body.data
                    {
                        self.check_function_overloads_recursive(&bd.statements.nodes);
                    }
                    if let Some(body) = &d.body
                        && body.kind == SyntaxKind::ModuleBlock
                        && let tsox_frontend::ast::NodeData::ModuleBlock(bd) = &body.data
                    {
                        self.check_function_overloads_recursive(&bd.statements.nodes);
                    }
                }
    ...
    pub(crate) fn check_statement_function_overloads(&mut self, statements: &[Arc<Node>]) {
        ...
        for (idx, s) in statements.iter().enumerate() {
            ...
            if let tsox_frontend::ast::NodeData::FunctionDeclaration(d) = &s.data
                && let Some(n) = &d.name
                && n.kind == SyntaxKind::Identifier
            {
        ...
        for (_, idxs) in groups {
            ...
            for &idx in &idxs {
                let body = matches!(
                    &statements[idx].data,
                    tsox_frontend::ast::NodeData::FunctionDeclaration(d) if d.body.is_some()
                );
                if !body {
                    if let Some(p) = prev {
                        if p + 1 != idx {
            ...
            if !has_body {
                ...
                let impl_required = {
                    let mut n = 0;
                    if let tsox_frontend::ast::NodeData::FunctionDeclaration(d) =
                        &statements[impl_idx].data
                    {
                        for p in d.parameters.iter() {
                ...
                if !impl_rest {
                    ...
                    for &i in &idxs {
                        ...
                        let shape = if let tsox_frontend::ast::NodeData::FunctionDeclaration(d) =
                            &statements[i].data
                        {
