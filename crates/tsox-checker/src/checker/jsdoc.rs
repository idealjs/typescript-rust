        ...
        if self.contains_arguments_reference(node) {
            if is_js {
                ...
                self.grammar_error_on_node_with_args(
                    last_name,
                    &tsox_core::diagnostics::messages_generated::JSDOC_PARAM_TAG_HAS_NAME_0_BUT_THERE_IS_NO_PARAMETER_WITH_THAT_NAME_IT_WOULD_MATCH_ARGUMENTS_IF_IT_HAD_AN_ARRAY_TYPE,
            ...
            for (index, tag) in jsdoc_parameters.iter().enumerate() {
                ...
                if name.kind == SyntaxKind::QualifiedName {
                    if is_js {
                        ...
                        self.grammar_error_on_node_with_args(
                            name,
                            &tsox_core::diagnostics::messages_generated::QUALIFIED_NAME_0_IS_NOT_ALLOWED_WITHOUT_A_LEADING_PARAM_OBJECT_1,
                            &[full, left],
                        );
                    }
                    ...
                    self.grammar_error_on_node_with_args(
                        name,
                        &tsox_core::diagnostics::messages_generated::JSDOC_PARAM_TAG_HAS_NAME_0_BUT_THERE_IS_NO_PARAMETER_WITH_THAT_NAME,
                        &[name.text().to_string()],
                    );
                }
            }
