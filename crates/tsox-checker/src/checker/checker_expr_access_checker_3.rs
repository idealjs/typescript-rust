...
impl Checker {
    ...
    pub(crate) fn check_private_identifier_access(
                ...
                if let (Some(lx), Some(type_class)) = (&lexical, &type_class) {
                    ...
                    if lexical_class.is_some_and(|lc| self.is_ancestor_class_of(&lc, type_class)) {
                        ...
                        self.diagnostics.add(tsox_frontend::ast::Diagnostic::new(
                            self.current_file.clone(),
                            name.loc,
                            tsox_core::diagnostics::messages_generated::THE_PROPERTY_0_CANNOT_BE_ACCESSED_ON_TYPE_1_WITHIN_THIS_CLASS_BECAUSE_IT_IS_SHADOWED_BY_ANOTHER_PRIVATE_IDENTIFIER_WITH_THE_SAME_SPELLING,
        ...
        if setonly && assignment_kind != crate::checker::utilities::AssignmentKind::Definite {
            self.diagnostics.add(tsox_frontend::ast::Diagnostic::new(
                self.current_file.clone(),
                node.loc,
                tsox_core::diagnostics::messages_generated::PRIVATE_ACCESSOR_WAS_DEFINED_WITHOUT_A_GETTER,
                vec![],
            ));
        }
        false
    ...
    pub(crate) fn function_body_definitely_returns(&self, body: &Arc<Node>) -> bool {
        ...
        if let tsox_frontend::ast::NodeData::Block(data) = &body.data {
    ...
    pub(crate) fn statement_always_returns(&self, stmt: &Arc<Node>) -> bool {
        match stmt.kind {
            ...
            SyntaxKind::Block => {
                if let tsox_frontend::ast::NodeData::Block(data) = &stmt.data {
                    if let Some(last) = data.statements.nodes.last() {
                        return self.statement_always_returns(last);
                    }
                }
            ...
            SyntaxKind::IfStatement => {
                if let tsox_frontend::ast::NodeData::IfStatement(data) = &stmt.data {
            ...
            SyntaxKind::WhileStatement | SyntaxKind::DoStatement => {
                let (condition, body) = match &stmt.data {
                    tsox_frontend::ast::NodeData::WhileStatement(data) => {
                        (&data.expression, &data.statement)
                    }
                    tsox_frontend::ast::NodeData::DoStatement(data) => {
                        (&data.expression, &data.statement)
                    }
                    _ => return false,
                };
            ...
            SyntaxKind::ForStatement => {
                if let tsox_frontend::ast::NodeData::ForStatement(data) = &stmt.data {
                    data.condition
                        .as_ref()
            ...
            SyntaxKind::SwitchStatement => {
                if let tsox_frontend::ast::NodeData::SwitchStatement(data) = &stmt.data
                    && let tsox_frontend::ast::NodeData::CaseBlock(block) = &data.case_block.data
                {
                    ...
                    block.clauses.iter().all(|c| match &c.data {
                        tsox_frontend::ast::NodeData::CaseOrDefaultClause(cd) => cd
                            .statements
                            .nodes
                            .last()
