...
pub(crate) fn module_format_is_esm_for_require_check(
    ...
    tsox_tsoptions::tsoptions::implied_node_format_of_file(path, read_file) == ModuleKind::ESNext
}
...
pub(crate) fn module_instance_state_worker(node: &Arc<Node>, visited: &mut Vec<usize>) -> u8 {
    match &node.data {
        ...
        tsox_frontend::ast::NodeData::EnumDeclaration(_) => {
        ...
        | tsox_frontend::ast::NodeData::ImportEqualsDeclaration(_) => {
        ...
        tsox_frontend::ast::NodeData::ExportDeclaration(ed) => {
                    ...
                    .is_some_and(|c| c.kind == SyntaxKind::NamedExports)
            {
                ...
                let tsox_frontend::ast::NodeData::NamedExports(named) = &clause.data else {
                    return 2;
                };
                let mut state = 0u8;
        ...
        tsox_frontend::ast::NodeData::ModuleDeclaration(md) => match &md.body {
            Some(body) => module_instance_state(body, visited),
            None => 2,
        },
        tsox_frontend::ast::NodeData::ModuleBlock(block) => {
            let mut state = 0u8;
