...
impl Checker {
    pub fn check_grammar_break_or_continue_statement(&mut self, node: &Arc<Node>) -> bool {
        ...
        for ctx in self.break_continue_context_stack.iter().rev() {
            match ctx.kind {
                crate::checker::grammarchecks_checker::BreakContinueContextKind::Function => {
                    return self
                        .grammar_error_on_node(node, &JUMP_TARGET_CANNOT_CROSS_FUNCTION_BOUNDARY);
                }
                crate::checker::grammarchecks_checker::BreakContinueContextKind::Labeled => {
                ...
                crate::checker::grammarchecks_checker::BreakContinueContextKind::Loop => {
                    if target_label.is_none() {
                        return false;
                    }
                }
                crate::checker::grammarchecks_checker::BreakContinueContextKind::Switch => {
