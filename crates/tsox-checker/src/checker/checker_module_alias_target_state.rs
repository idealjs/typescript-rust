    ...
    while let Some(p) = anc {
        if matches!(
            ...
            let stmts: &[Arc<Node>] = match &p.data {
                tsox_frontend::ast::NodeData::ModuleBlock(b) => &b.statements.nodes,
                tsox_frontend::ast::NodeData::SourceFile(sf) => &sf.statements.nodes,
                tsox_frontend::ast::NodeData::Block(b) => &b.statements.nodes,
                _ => &[],
            };
...
pub(crate) fn statement_declares_name(stmt: &Arc<Node>, id_text: &str) -> bool {
    let name: Option<&Arc<Node>> = match &stmt.data {
        ...
        tsox_frontend::ast::NodeData::EnumDeclaration(e) => Some(&e.name),
        tsox_frontend::ast::NodeData::ModuleDeclaration(m) => Some(&m.name),
        tsox_frontend::ast::NodeData::InterfaceDeclaration(i) => Some(&i.name),
        tsox_frontend::ast::NodeData::TypeAliasDeclaration(t) => Some(&t.name),
        tsox_frontend::ast::NodeData::ImportEqualsDeclaration(i) => Some(&i.name),
        tsox_frontend::ast::NodeData::VariableStatement(vs) => {
            let tsox_frontend::ast::NodeData::VariableDeclarationList(dl) =
...
pub(crate) fn binding_names_cover(decl: &Arc<Node>, id_text: &str) -> bool {
    ...
    match &d.name.kind {
        ...
        SyntaxKind::ObjectBindingPattern | SyntaxKind::ArrayBindingPattern => {
            let mut hit = false;
            tsox_frontend::ast::node_data_generated::for_each_child(&d.name, |el| {
