...
pub(crate) fn is_assignment_target(node: &Arc<Node>) -> bool {
    ...
    if parent.kind == SyntaxKind::BindingElement {
        if let tsox_frontend::ast::NodeData::BindingElement(be) = &parent.data {
    ...
    let tsox_frontend::ast::NodeData::BinaryExpression(bin) = &parent.data else {
        return false;
    };
...
pub(crate) fn body_assigns_this_property(n: &Arc<Node>, name: &str) -> bool {
    match &n.data {
        ...
        {
            if let tsox_frontend::ast::NodeData::PropertyAccessExpression(pa) = &b.left.data
        ...
        tsox_frontend::ast::NodeData::FunctionDeclaration(_)
        | tsox_frontend::ast::NodeData::FunctionExpression(_)
        | tsox_frontend::ast::NodeData::ArrowFunction(_) => return false,
        _ => {}
    }
    let mut found = false;
