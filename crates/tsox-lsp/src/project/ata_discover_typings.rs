...
pub struct TypeAcquisition {
    ...
    pub disable_filename_based_type_acquisition: tsox_core::core::tristate::Tristate,
}
...
pub struct CachedTyping {
    ...
    pub version: tsox_core::semver::Version,
}
...
pub fn is_typing_up_to_date(
    ...
    let available_version = tsox_core::semver::must_parse(use_version);
...
pub fn discover_typings(
    ...
    let js_file_names: Vec<&String> = file_names
        .iter()
        .filter(|f| tsox_core::tspath::has_js_file_extension(f))
        .collect();
...
pub(crate) fn get_typing_names_from_source_file_names(
    ...
    for file_name in file_names {
        has_jsx_file = has_jsx_file
            || tsox_core::tspath::file_extension_is(file_name, tsox_core::tspath::EXTENSION_JSX);
        let inferred_typing_name =
            tsox_core::tspath::remove_file_extension(&tsox_core::tspath::to_file_name_lower_case(
                &tsox_core::tspath::get_base_file_name(file_name),
            ));
