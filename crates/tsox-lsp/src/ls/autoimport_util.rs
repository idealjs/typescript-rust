...
pub fn get_package_names_in_node_modules(node_modules_dir: &str, fs: &dyn FS) -> Set<String> {
    ...
    for base_name in &entries.directories {
        ...
        if base_name.starts_with('@') {
            let scoped_dir_path = tsox_core::tspath::combine_paths(node_modules_dir, &[base_name]);
            let scoped_entries = fs.get_accessible_entries(&scoped_dir_path);
            for scoped_package_dir_name in &scoped_entries.directories {
                let scoped_base_name =
                    tsox_core::tspath::get_base_file_name(scoped_package_dir_name);
                if base_name == "@types" {
                    package_names.add(
                        tsox_tsoptions::module::get_package_name_from_types_package_name(
                            &tsox_core::tspath::combine_paths("@types", &[&scoped_base_name]),
                        ),
                    );
                } else {
                    package_names.add(tsox_core::tspath::combine_paths(
                        base_name,
                        &[&scoped_base_name],
                    ));
                }
...
pub fn get_package_realpath_funcs(
    ...
    let to_realpath: Box<dyn Fn(&str) -> String + Send + Sync> = {
        ...
        Box::new(move |file_name: &str| {
            ...
            let pkg_dir = tsox_tsoptions::module::parse_node_module_from_path(file_name, false);
            if pkg_dir.is_empty() {
