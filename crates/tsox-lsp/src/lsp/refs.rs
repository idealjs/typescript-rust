...
impl LspServer {
    pub(super) fn handle_references(&self, params: &Value) -> Value {
        ...
        let target_symbol =
            match tsox_frontend::astnav::get_token_at_position(&source_file.node, offset) {
        ...
        for (sf, node) in &refs {
            ...
            let (el, ec) = tsox_frontend::diagnosticwriter::line_and_character(
                &sf.line_map,
                &sf.text,
                node.end(),
            );
    ...
    pub(super) fn handle_document_symbol(&self, params: &Value) -> Value {
        ...
        let statements = match &source_file.node.data {
            tsox_frontend::ast::NodeData::SourceFile(data) => data.statements.nodes.as_slice(),
            _ => return json!([]),
        };
