...
impl LspServer {
    ...
    pub(super) fn compute_all_diagnostics(&self) -> Vec<Value> {
        ...
        let mut all_diags: Vec<tsox_frontend::ast::diagnostic::Diagnostic> = Vec::new();
        for d in program.get_diagnostics_to_report() {
            all_diags.push((*d).clone());
        }
        all_diags.extend(program.get_semantic_diagnostics());

        let mut by_path: HashMap<&str, Vec<&tsox_frontend::ast::diagnostic::Diagnostic>> =
            HashMap::new();
        let mut fileless: Vec<&tsox_frontend::ast::diagnostic::Diagnostic> = Vec::new();
        for d in &all_diags {
            match &d.file {
