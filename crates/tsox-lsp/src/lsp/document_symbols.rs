...
fn document_symbol_for_node(
    ...
    let (rl, rc) =
        tsox_frontend::diagnosticwriter::line_and_character(&sf.line_map, &sf.text, node.pos());
    let (rel, rec) =
        tsox_frontend::diagnosticwriter::line_and_character(&sf.line_map, &sf.text, node.end());
    let (sl, sc) = tsox_frontend::diagnosticwriter::line_and_character(
        &sf.line_map,
        &sf.text,
        name_node.pos(),
    );
    let (sel, sec) = tsox_frontend::diagnosticwriter::line_and_character(
        &sf.line_map,
        &sf.text,
        name_node.end(),
    );
