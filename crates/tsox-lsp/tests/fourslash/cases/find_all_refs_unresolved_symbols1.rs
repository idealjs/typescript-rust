use tsox_lsp::fourslash::{self, Session};


#[ignore = "unimplemented: fourslash.VerifyBaselineFindAllReferences"]
#[test]
fn find_all_refs_unresolved_symbols1() {
    let content = r#"let a: /*a0*/Bar;
let b: /*a1*/Bar<string>;
let c: /*a2*/Bar<string, number>;
let d: /*b0*/Bar./*c0*/X;
let e: /*b1*/Bar./*c1*/X<string>;
let f: /*b2*/Bar./*d0*/X./*e0*/Y;"#;
    let mut s = Session::new_for_test("findAllRefsUnresolvedSymbols1", content);
    fourslash::unsupported("VerifyBaselineFindAllReferences"); // f.VerifyBaselineFindAllReferences(t, "a0", "a1", "a2", "b0", "b1", "b2", "c0", "c1", "d0", "e0")
}
