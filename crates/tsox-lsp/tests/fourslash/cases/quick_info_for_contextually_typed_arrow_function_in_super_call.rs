...
class B extends A<number, string> {
    ...
    let mut s = Session::new_for_test("quickInfoForContextuallyTypedArrowFunctionInSuperCall", content);
    ...
    fourslash::verify_quick_info_at(&mut s, "3", "(method) Number.toExponential(fractionDigits?: number): string", "Returns a string containing a number represented in exponential notation.");
}
