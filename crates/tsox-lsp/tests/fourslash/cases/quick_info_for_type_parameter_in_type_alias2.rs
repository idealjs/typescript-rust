...
type Nesting<TT> = { method<UU>(): new <WW>() => T/*5*/T & U/*6*/U & W/*7*/W };"#;
    let mut s = Session::new_for_test("quickInfoForTypeParameterInTypeAlias2", content);
    ...
    fourslash::verify_quick_info_at(&mut s, "3", "(type parameter) AA in type GenericMethod<AA>", "");
    fourslash::verify_quick_info_at(&mut s, "4", "(type parameter) BB in method<BB>(): AA & BB", "");
    fourslash::verify_quick_info_at(&mut s, "5", "(type parameter) TT in type Nesting<TT>", "");
    fourslash::verify_quick_info_at(&mut s, "6", "(type parameter) UU in method<UU>(): new <WW>() => TT & UU & WW", "");
    fourslash::verify_quick_info_at(&mut s, "7", "(type parameter) WW in <WW>(): TT & UU & WW", "");
}
