...
type Construct<AA> = { new(): A/*5*/A };"#;
    let mut s = Session::new_for_test("quickInfoForTypeParameterInTypeAlias1", content);
    ...
    fourslash::verify_quick_info_at(&mut s, "3", "(type parameter) AA in type NestedCtor<AA>", "");
