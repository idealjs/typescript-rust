...
/*5*/				to determine whether checkbox has focus or not
    ...
    fourslash::verify_current_line_content(
        &mut s,
        r#"            /* when current checkbox has focus, Firefox has changed check state already"#,
    );
    ...
    fourslash::verify_current_line_content(
        &mut s,
        r#"            IE does not have issue, use the CSS class"#,
    );
    fourslash::go_to_marker(&mut s, "4");
    fourslash::verify_current_line_content(
        &mut s,
        r#"            input:focus[type=checkbox] (z-index = 31290)"#,
    );
    fourslash::go_to_marker(&mut s, "5");
    fourslash::verify_current_line_content(
        &mut s,
        r#"            to determine whether checkbox has focus or not"#,
    );
}
