...
var /*1*/result = /*2*/f(0, /*3*/x => null, /*4*/x => x.blahblah);"#;
    let mut s = Session::new_for_test("fixingTypeParametersQuickInfo", content);
    fourslash::verify_quick_info_at(&mut s, "1", "var result: number", "");
    fourslash::verify_quick_info_at(&mut s, "2", "function f<number>(x: number, y: (p: number) => number, z: (p: number) => number): number", "");
