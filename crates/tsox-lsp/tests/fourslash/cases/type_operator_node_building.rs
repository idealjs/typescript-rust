...
const /*2*/utilityFunctions = {
  doSomethingWithTypes
    ...
    let mut s = Session::new_for_test("typeOperatorNodeBuilding", content);
    fourslash::verify_quick_info_at(&mut s, "1", "const utilityFunctions: {\n    doSomethingWithKeys: <T>(...keys: (keyof T)[]) => void;\n}", "");
    fourslash::verify_quick_info_at(&mut s, "2", "const utilityFunctions: {\n    doSomethingWithTypes: (...statics: (typeof Foo)[]) => void;\n}", "");
}
