...
impl<'a> RegExpParser<'a> {
    pub(crate) fn scan_atom_escape(&mut self) {
        ...
        if ch == 'k' {
            ...
            if self.char() == '<' {
                ...
                self.error(
                    tsox_core::diagnostics::X_K_MUST_BE_FOLLOWED_BY_A_CAPTURING_GROUP_NAME_ENCLOSED_IN_ANGLE_BRACKETS,
                    self.pos - 2,
                    2,
                );
            }
        ...
        if ch == 'q' && self.unicode_sets_mode {
            self.inc_pos(1);
            self.error(
                tsox_core::diagnostics::X_Q_IS_ONLY_AVAILABLE_INSIDE_CHARACTER_CLASS,
                self.pos - 2,
                2,
            );
            return;
        ...
        match ch {
            'c' => {
                ...
                if self.any_unicode_mode_or_non_annex_b {
                    self.error(
                        tsox_core::diagnostics::X_C_MUST_BE_FOLLOWED_BY_AN_ASCII_LETTER,
                        self.pos - 2,
                        2,
                    );
                } else if atom_escape {
    ...
    pub(super) fn scan_escape_sequence(
        ...
        match ch {
            ...
            '8' | '9' => {
                if !atom_escape {
                    self.error(
                        tsox_core::diagnostics::DECIMAL_ESCAPE_SEQUENCES_AND_BACKREFERENCES_ARE_NOT_ALLOWED_IN_A_CHARACTER_CLASS,
                        start,
                        self.pos - start,
                    );
                }
            ...
            'u' => {
                if self.char() == '{' {
                    ...
                    if !any_unicode_mode {
                        self.error(
                            tsox_core::diagnostics::UNICODE_ESCAPE_SEQUENCES_ARE_ONLY_AVAILABLE_WHEN_THE_UNICODE_U_FLAG_OR_THE_UNICODE_SETS_V_FLAG_IS_SET,
                            start,
                            self.pos - start,
                        );
                    }
            ...
            _ => {
                ...
                if any_unicode_mode || (!annex_b && is_identifier_part(c)) {
                    self.error(
                        tsox_core::diagnostics::THIS_CHARACTER_CANNOT_BE_ESCAPED_IN_A_REGULAR_EXPRESSION,
                        start,
                        self.pos - start,
                    );
                }
    ...
    pub(crate) fn scan_group_name(&mut self, is_reference: bool) {
        ...
        if self.pos == token_start {
            self.error(
                tsox_core::diagnostics::EXPECTED_A_CAPTURING_GROUP_NAME,
                self.pos,
                0,
            );
            ...
            self.error(
                tsox_core::diagnostics::NAMED_CAPTURING_GROUPS_WITH_THE_SAME_NAME_MUST_BE_MUTUALLY_EXCLUSIVE_TO_EACH_OTHER,
                token_start,
