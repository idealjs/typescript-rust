...
use crate::scanner::regexp::REG_EXP_FLAG_MODIFIERS;
use crate::scanner::regexp::RegExpParser;
use crate::scanner::regexp::char_to_reg_exp_flag;
use crate::scanner::regexp::compare_decimal_strings;
use crate::scanner::regexp::decode_rune_at;
use crate::scanner::regexp::is_digit;
            ...
            match ch {
                                ...
                                match self.char() {
                                    ...
                                    _ => {
                                        ...
                                        if self.script_target < ScriptTarget::ES2018 {
                                            self.error(
                                                tsox_core::diagnostics::NAMED_CAPTURING_GROUPS_ARE_ONLY_AVAILABLE_WHEN_TARGETING_ES2018_OR_LATER,
                                                group_name_start,
                            ...
                            _ => {
                                ...
                                if self.char() == '-' {
                                    ...
                                    if self.pos == flags_start + 1 {
                                        self.error(
                                            tsox_core::diagnostics::SUBPATTERN_FLAGS_MUST_BE_PRESENT_WHEN_THERE_IS_A_MINUS_SIGN,
                                            flags_start,
                ...
                '{' => {
                    ...
                    if self.char() == ',' {
                        ...
                        if min_str.is_empty() {
                            if !max_str.is_empty() || self.char() == '}' {
                                self.error(
                                    tsox_core::diagnostics::INCOMPLETE_QUANTIFIER_DIGIT_EXPECTED,
                                    digits_start,
                                    0,
                                );
                            } else {
                            ...
                            {
                                self.error(
                                    tsox_core::diagnostics::NUMBERS_OUT_OF_ORDER_IN_QUANTIFIER,
                                    digits_start,
                    ...
                    if !is_previous_term_quantifiable {
                        self.error(
                            tsox_core::diagnostics::THERE_IS_NOTHING_AVAILABLE_FOR_REPETITION,
                            start,
                            self.pos - start,
                        );
                    }
    ...
    pub(super) fn scan_pattern_modifiers(&mut self, curr_flags: u16) -> u16 {
        ...
        while self.pos < self.body_end {
            ...
            if flag == 0 {
                ...
                self.error(
                    tsox_core::diagnostics::THIS_REGULAR_EXPRESSION_FLAG_CANNOT_BE_TOGGLED_WITHIN_A_SUBPATTERN,
                    self.pos,
                    size,
                );
            } else {
