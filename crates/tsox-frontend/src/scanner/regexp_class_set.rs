...
impl<'a> RegExpParser<'a> {
    pub(super) fn scan_class_set_expression(&mut self) {
        ...
        if matches!(two, Some([b'-', b'-']) | Some([b'&', b'&'])) {
            self.error(
                tsox_core::diagnostics::EXPECTED_A_CLASS_SET_OPERAND,
                self.pos,
                0,
            );
            ...
            match ch {
                '-' => {
                    ...
                    if ch2 == '-' {
                        self.inc_pos(1);
                        self.error(
                            tsox_core::diagnostics::OPERATORS_MUST_NOT_BE_MIXED_WITHIN_A_CHARACTER_CLASS_WRAP_IT_IN_A_NESTED_CLASS_INSTEAD,
                            self.pos - 2,
                            2,
                        );
                        start = self.pos - 2;
                        ...
                        if operand.is_empty() {
                            self.error(
                                tsox_core::diagnostics::A_CHARACTER_CLASS_RANGE_MUST_NOT_BE_BOUNDED_BY_ANOTHER_CHARACTER_CLASS,
                                start,
                                self.pos - 1 - start,
                            );
                        }
                        ...
                        if second_operand.is_empty() {
                            ...
                            if let (Some((min_c, min_size)), Some((max_c, max_size))) = (
                                ...
                                {
                                    self.error(
                                        tsox_core::diagnostics::RANGE_OUT_OF_ORDER_IN_CHARACTER_CLASS,
                                        start,
                                        self.pos - start,
                                    );
                                }
