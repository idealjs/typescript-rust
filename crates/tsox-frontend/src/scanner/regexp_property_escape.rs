        ...
        if self.char() == '{' {
            ...
            if self.char() == '=' {
                ...
                if self.pos == property_name_or_value_start {
                    self.error(
                        tsox_core::diagnostics::EXPECTED_A_UNICODE_PROPERTY_NAME,
                        self.pos,
                        0,
                    );
                } else if property_name.is_empty() {
                    self.error(
                        tsox_core::diagnostics::UNKNOWN_UNICODE_PROPERTY_NAME,
                ...
                if self.pos == property_value_start {
                    self.error(
                        tsox_core::diagnostics::EXPECTED_A_UNICODE_PROPERTY_VALUE,
                        self.pos,
                        0,
                    );
                } else if !property_name.is_empty() {
                    if !unicode_properties::is_valid_unicode_property_value(
                        ...
                        self.error(
                            tsox_core::diagnostics::UNKNOWN_UNICODE_PROPERTY_VALUE,
                ...
                if self.pos == property_name_or_value_start {
                    self.error(
                        tsox_core::diagnostics::EXPECTED_A_UNICODE_PROPERTY_NAME_OR_VALUE,
                        self.pos,
                        0,
                    );
                    ...
                    if !self.unicode_sets_mode {
                        self.error(
                            tsox_core::diagnostics::ANY_UNICODE_PROPERTY_THAT_WOULD_POSSIBLY_MATCH_MORE_THAN_A_SINGLE_CHARACTER_IS_ONLY_AVAILABLE_WHEN_THE_UNICODE_SETS_V_FLAG_IS_SET,
                            property_name_or_value_start,
                            self.pos - property_name_or_value_start,
                        );
                    } else if is_character_complement {
                        self.error(
                            tsox_core::diagnostics::ANYTHING_THAT_WOULD_POSSIBLY_MATCH_MORE_THAN_A_SINGLE_CHARACTER_IS_INVALID_INSIDE_A_NEGATED_CHARACTER_CLASS,
                            property_name_or_value_start,
                            self.pos - property_name_or_value_start,
                        );
                    } else {
                    ...
                    self.error(
                        tsox_core::diagnostics::UNKNOWN_UNICODE_PROPERTY_NAME_OR_VALUE,
                        property_name_or_value_start,
                        self.pos - property_name_or_value_start,
                    );
                }
            ...
            if !self.any_unicode_mode {
                self.error(
                    tsox_core::diagnostics::UNICODE_PROPERTY_VALUE_EXPRESSIONS_ARE_ONLY_AVAILABLE_WHEN_THE_UNICODE_U_FLAG_OR_THE_UNICODE_SETS_V_FLAG_IS_SET,
                    start,
                    self.pos - start,
                );
            }
            ...
            self.error(
                tsox_core::diagnostics::X_0_MUST_BE_FOLLOWED_BY_A_UNICODE_PROPERTY_VALUE_EXPRESSION_ENCLOSED_IN_BRACES,
                self.pos - 2,
                2,
            );
        } else {
