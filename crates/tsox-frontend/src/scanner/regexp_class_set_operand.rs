            ...
            '\\' => {
                ...
                if self.scan_character_class_escape() {
                    ...
                    if self.char() == '{' {
                        ...
                        self.error(
                            tsox_core::diagnostics::X_Q_MUST_BE_FOLLOWED_BY_STRING_ALTERNATIVES_ENCLOSED_IN_BRACES,
                            self.pos - 2,
                            2,
                        );
                        return "q".to_string();
    ...
    pub(super) fn scan_class_set_character(&mut self) -> String {
        ...
        if ch == '\\' {
            ...
            match ch {
                ...
                | '?' | '@' | '`' | '~' => {
                    self.error(
                        tsox_core::diagnostics::A_CHARACTER_CLASS_MUST_NOT_CONTAIN_A_RESERVED_DOUBLE_PUNCTUATOR_DID_YOU_MEAN_TO_ESCAPE_IT_WITH_BACKSLASH,
                        self.pos,
                        2,
                    );
                    self.inc_pos(2);
