use crate::ast::*;
use crate::parser::reparser_namespace::deep_clone;
use crate::parser::reparser_namespace::make_question_if_optional;
use std::sync::Arc;
...
pub(super) fn property_tags_to_signatures(tags: &[Arc<Node>]) -> Vec<Arc<Node>> {
    ...
    for prop in tags {
        ...
        let prop_type = type_expression.as_ref().and_then(|te| match &te.data {
            NodeData::JSDocTypeExpression(ted) => Some(reparse_jsdoc_type_literal(&ted.type_node)),
            _ => None,
        });
...
pub(super) fn reparse_jsdoc_type_literal(t: &Arc<Node>) -> Arc<Node> {
    ...
    if let Some(tags) = property_tags {
        properties = property_tags_to_signatures(tags);
    }
