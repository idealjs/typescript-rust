        ...
        match context {
            ParsingContext::SourceElements => {
                if self.token == SyntaxKind::DefaultKeyword {
                    ...
                    self.parse_error_at_current_token(
                        tsox_core::diagnostics::DECLARATION_OR_STATEMENT_EXPECTED,
                        &[],
                    );
                }
            }
            ...
            ParsingContext::SwitchClauses => {
                self.parse_error_at_current_token(
                    tsox_core::diagnostics::X_CASE_OR_DEFAULT_EXPECTED,
                    &[],
                );
            }
            ParsingContext::SwitchClauseStatements => {
                self.parse_error_at_current_token(tsox_core::diagnostics::STATEMENT_EXPECTED, &[]);
            }
            ParsingContext::RestProperties | ParsingContext::TypeMembers => {
                self.parse_error_at_current_token(
                    tsox_core::diagnostics::PROPERTY_OR_SIGNATURE_EXPECTED,
                    &[],
                );
            }
            ParsingContext::ClassMembers => {
                self.parse_error_at_current_token(
                    tsox_core::diagnostics::UNEXPECTED_TOKEN_A_CONSTRUCTOR_METHOD_ACCESSOR_OR_PROPERTY_WAS_EXPECTED,
                    &[],
                );
            }
            ParsingContext::EnumMembers => {
                self.parse_error_at_current_token(
                    tsox_core::diagnostics::ENUM_MEMBER_EXPECTED,
                    &[],
                );
            }
            ...
            ParsingContext::VariableDeclarations => {
                if is_keyword_kind(self.token) {
                    self.parse_error_at_current_token(
                        tsox_core::diagnostics::X_0_IS_NOT_ALLOWED_AS_A_VARIABLE_DECLARATION_NAME,
                        &[token_to_string(self.token)],
                    );
                } else {
                    self.parse_error_at_current_token(
                        tsox_core::diagnostics::VARIABLE_DECLARATION_EXPECTED,
                        &[],
                    );
                }
            }
            ParsingContext::ObjectBindingElements => {
                self.parse_error_at_current_token(
                    tsox_core::diagnostics::PROPERTY_DESTRUCTURING_PATTERN_EXPECTED,
                    &[],
                );
            }
            ParsingContext::ArrayBindingElements => {
                self.parse_error_at_current_token(
                    tsox_core::diagnostics::ARRAY_ELEMENT_DESTRUCTURING_PATTERN_EXPECTED,
                    &[],
                );
            }
            ParsingContext::ArgumentExpressions => {
                self.parse_error_at_current_token(
                    tsox_core::diagnostics::ARGUMENT_EXPRESSION_EXPECTED,
                    &[],
                );
            }
            ...
            ParsingContext::ArrayLiteralMembers => {
                self.parse_error_at_current_token(
                    tsox_core::diagnostics::EXPRESSION_OR_COMMA_EXPECTED,
                    &[],
                );
            }
            ParsingContext::JSDocParameters => {
                self.parse_error_at_current_token(
                    tsox_core::diagnostics::PARAMETER_DECLARATION_EXPECTED,
                    &[],
                );
            }
            ParsingContext::Parameters => {
                if is_keyword_kind(self.token) {
                    self.parse_error_at_current_token(
                        tsox_core::diagnostics::X_0_IS_NOT_ALLOWED_AS_A_PARAMETER_NAME,
                        &[token_to_string(self.token)],
                    );
                } else {
                    self.parse_error_at_current_token(
            ...
            ParsingContext::TypeParameters => {
                self.parse_error_at_current_token(
                    tsox_core::diagnostics::TYPE_PARAMETER_DECLARATION_EXPECTED,
                    &[],
                );
            }
            ParsingContext::TypeArguments => {
                self.parse_error_at_current_token(
                    tsox_core::diagnostics::TYPE_ARGUMENT_EXPECTED,
                    &[],
                );
            }
            ParsingContext::TupleElementTypes => {
                self.parse_error_at_current_token(tsox_core::diagnostics::TYPE_EXPECTED, &[]);
            }
            ParsingContext::HeritageClauses => {
                self.parse_error_at_current_token(
                    tsox_core::diagnostics::UNEXPECTED_TOKEN_EXPECTED,
                    &[],
                );
            }
            ...
            ParsingContext::ImportAttributes => {
                self.parse_error_at_current_token(
                    tsox_core::diagnostics::IDENTIFIER_OR_STRING_LITERAL_EXPECTED,
                    &[],
                );
            }
        }
