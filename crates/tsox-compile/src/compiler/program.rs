...

        {
            ...
            let expanded_types: Vec<String> = if options.types.iter().any(|t| t == "*") {
                let (type_roots, _from_config) =
                    tsox_tsoptions::module::resolver::get_effective_type_roots(&options, host.current_directory());
            ...
            while let Some(file) = stack.pop() {
                    ...
                    && (file.file_name.ends_with(".tsx") || file.file_name.ends_with(".jsx"))
                {
                    ...
                    let mode =
                        tsox_tsoptions::tsoptions::implied_node_format_of_file(&file.file_name, &|p| host.fs().read_file(p));
