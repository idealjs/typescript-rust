...
pub(crate) const INPUT_DIRECTORY: &str = "/";

pub(crate) const LIB_DIRECTORY: &str = "/lib";

pub(crate) const BAREBONES_LIB_CONTENT: &str = "interface Boolean {}
interface Function {}
...
pub fn transpile_worker(
    ...
    let input_file_name =
        tsox_core::tspath::get_normalized_absolute_path(&file_name, INPUT_DIRECTORY);
    ...
    if declaration {
        ...
        let lib_path = tsox_core::tspath::combine_paths(LIB_DIRECTORY, &[&lib_file_name]);
        fs.insert_dir(LIB_DIRECTORY);
        fs.insert_file(&lib_path, BAREBONES_LIB_CONTENT);
    }

    let host = Arc::new(CompilerHostImpl::new(
        fs.clone() as Arc<dyn tsox_tsoptions::vfs::FS>,
