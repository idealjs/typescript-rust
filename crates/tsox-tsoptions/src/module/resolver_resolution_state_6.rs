...
impl<'a> ResolutionState<'a> {
    pub(crate) fn try_file(&self, file_name: &str) -> Option<String> {
        ...
        let ext = tsox_core::tspath::try_get_extension_from_path(file_name);
        let file_name_no_ext = tsox_core::tspath::remove_extension(file_name, ext);
    ...
    pub(crate) fn get_paths_base_path(&self) -> String {
        ...
        if !self.compiler_options.config_file_path.is_empty() {
            return tsox_core::tspath::get_directory_path(&self.compiler_options.config_file_path);
        }
        ...
        if let Some(matched_pattern) = match_pattern_or_exact(parsed_patterns, module_name) {
            ...
            if let Some(substitutions) = paths.get(&matched_pattern.text) {
                for subst in substitutions {
                    ...
                    let candidate = tsox_core::tspath::normalize_path(
                        &tsox_core::tspath::combine_paths(containing_directory, &[&path]),
                    );

                    let extension_from_subst =
                        tsox_core::tspath::try_get_extension_from_path(subst);
    ...
    pub(crate) fn try_load_module_using_root_dirs(&mut self) -> Option<Resolved> {
        ...
        for root_dir in &self.compiler_options.root_dirs {
            let mut normalized_root = tsox_core::tspath::normalize_path(root_dir);
        ...
        let matched_root_normalized = tsox_core::tspath::normalize_path(
            &matched_normalized_prefix[..matched_normalized_prefix.len().saturating_sub(1)],
        );
        for root_dir in &self.compiler_options.root_dirs {
            let normalized = tsox_core::tspath::normalize_path(root_dir);
            if normalized == matched_root_normalized {
                continue;
            }
            let alternate = tsox_core::tspath::normalize_path(&tsox_core::tspath::combine_paths(
                &normalized,
                &[suffix],
            ));
            if let Some(r) =
    ...
    pub(crate) fn resolve_from_type_root(&mut self) -> Option<Resolved> {
        ...
        for type_root in &type_roots {
            ...
            let package_directory = tsox_core::tspath::combine_paths(type_root, &[&self.name]);
    ...
    pub(crate) fn resolve_type_reference_directive(
        ...
        if !from_config || !from_inferred_types_containing_file {
            let resolved = if tsox_core::tspath::is_external_module_name_relative(&self.name) {
                let candidate =
                    Self::normalize_path_for_cjs_resolution(&self.containing_directory, &self.name);
