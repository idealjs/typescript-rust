...
impl<'a> ResolutionState<'a> {
    pub(crate) fn get_candidate_from_type_root(&self, type_root: &str) -> String {
        ...
        {
            crate::module::resolver::mangle_scoped_package_name(&self.name)
        } else {
            self.name.clone()
        };
        tsox_core::tspath::combine_paths(type_root, &[&name_for_lookup])
    }
    ...
    pub(crate) fn load_module_from_nearest_node_modules_directory_worker(
        ...
        loop {
            ...
            let parent = tsox_core::tspath::get_directory_path(&directory);
            if parent == directory {
                break;
            }
            directory = parent;
    ...
    pub(crate) fn load_module_from_immediate_node_modules_directory(
        ...
        if ext.contains(Extensions::DECLARATION) {
            let node_modules_at_types = tsox_core::tspath::combine_paths(&node_modules_folder, &["@types"]);
    ...
    pub(crate) fn load_module_from_specific_node_modules_directory(
        ...
        let package_directory = tsox_core::tspath::combine_paths(node_modules_directory, &[&package_name]);
        ...
        if package_info_exists && !has_exports && self.esm_mode {
            let index = tsox_core::tspath::combine_paths(&candidate, &["index.js"]);
    ...
    pub(crate) fn load_node_module_from_directory_worker(
        ...
        if !self.esm_mode {
            let index = tsox_core::tspath::combine_paths(candidate, &["index"]);
            if let Some(resolved) = self.load_module_from_file(ext, &index) {
                return Some(resolved);
            }
        }
