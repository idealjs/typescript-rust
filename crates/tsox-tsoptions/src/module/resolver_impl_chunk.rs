...
impl Extensions {
    ...
    pub fn array(&self) -> Vec<&'static str> {
        ...
        if self.contains(Extensions::TYPESCRIPT) {
            result.extend_from_slice(&tsox_core::tspath::SUPPORTED_TS_IMPLEMENTATION_EXTENSIONS);
        }
        ...
        if self.contains(Extensions::DECLARATION) {
            result.extend_from_slice(&tsox_core::tspath::SUPPORTED_DECLARATION_EXTENSIONS);
        }
        if self.contains(Extensions::JSON) {
            result.push(tsox_core::tspath::EXTENSION_JSON);
        }
        result
    }
