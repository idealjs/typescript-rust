...
pub(crate) fn get_include_base_path(absolute: &str) -> String {
    ...
    match wildcard_offset {
        None => {
            if !tsox_core::tspath::has_extension(absolute) {
                absolute.to_string()
            } else {
                tsox_core::tspath::remove_trailing_directory_separator(
                    &tsox_core::tspath::get_directory_path(absolute),
                )
            }
        }
        Some(woff) => {
...
pub fn get_base_paths(
    ...
    if !includes.is_empty() {
        let options = tsox_core::tspath::ComparePathsOptions {
        ...
        for include in includes {
            let absolute = if tsox_core::tspath::is_rooted_disk_path(include) {
                include.to_string()
            } else {
...
pub(crate) fn contains_path(
    ...
    let child_components = tsox_core::tspath::reduce_path_components(
        &tsox_core::tspath::get_path_components(child, &options.current_directory),
    );
...
pub fn compile_glob_pattern(
    ...
    if let Some(first) = parts.first_mut() {
        *first = tsox_core::tspath::remove_trailing_directory_separator(first);
    }
