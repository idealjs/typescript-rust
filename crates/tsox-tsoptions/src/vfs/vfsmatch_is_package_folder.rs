...
impl<'a> GlobVisitor<'a> {
    pub(crate) fn visit(
        ...
        let canonical_path = tsox_core::tspath::get_canonical_file_name(
            &real_path,
        ...
        for file in &entries.files {
            if !self.extensions.is_empty()
                && !tsox_core::tspath::file_extension_is_one_of(file, self.extensions)
            {
                continue;
            }
        ...
        for dir in &entries.directories {
            ...
            let child_real_path = if !is_symlink {
                tsox_core::tspath::combine_paths(&real_path, &[dir])
            } else {
                String::new()
            };
            self.visit(
...
pub fn match_files(
    ...
    for base_path in get_base_paths(&path, includes, use_case_sensitive_file_names) {
        let abs = tsox_core::tspath::combine_paths(&current_directory, &[&base_path]);
