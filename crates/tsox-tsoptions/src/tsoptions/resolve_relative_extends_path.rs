...
pub(crate) fn resolve_relative_extends_path(
    ...
    let dir_form = tsox_core::tspath::combine_paths(&base, &["tsconfig.json"]);
    if fs.file_exists(&dir_form) {
        return Some(dir_form);
    }

    let abs = tsox_core::tspath::get_normalized_absolute_path(s, current_dir);
    if fs.file_exists(&abs) {
        Some(abs)
    } else {
        Some(tsox_core::tspath::combine_paths(&abs, &["tsconfig.json"]))
    }
}
...
pub(crate) fn resolve_config_via_node_modules(
    ...
    tsox_core::tspath::for_each_ancestor_directory(containing_directory, |ancestor| {
        ...
        let node_modules = tsox_core::tspath::combine_paths(ancestor, &["node_modules"]);
...
pub(crate) fn load_config_from_node_modules(
    ...
    let tsconfig_in_dir = tsox_core::tspath::combine_paths(&candidate, &["tsconfig.json"]);
    if fs.file_exists(&tsconfig_in_dir) {
        return Some(tsconfig_in_dir);
    }

    let package_dir = tsox_core::tspath::combine_paths(node_modules_dir, &[&package_name]);
    let package_json_path = tsox_core::tspath::combine_paths(&package_dir, &["package.json"]);
        ...
        if let Some(content) = fs.read_file(&package_json_path) {
            if let Ok(fields) = crate::packagejson::parse(&content) {
                if let Some(tsconfig_field) = fields.path_fields.tsconfig.get_value() {
                    let resolved = tsox_core::tspath::get_normalized_absolute_path(
                        tsconfig_field,
                        &package_dir,
                    );
...
pub(crate) fn json_to_opt_value(v: &tsox_core::json::Value) -> OptValue {
    match v {
        ...
        tsox_core::json::Value::Array(arr) => {
            let list = arr
                .iter()
        ...
        tsox_core::json::Value::Null => OptValue::Null,
        tsox_core::json::Value::Object(_) => OptValue::Null,
    }
}
...
pub(crate) fn resolve_file_path_options(options: &mut CompilerOptions, base_path: &str) {
    let resolve = |s: &str| -> String {
        ...
        tsox_core::tspath::get_normalized_absolute_path(s, base_path)
    };
