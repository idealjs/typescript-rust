...
pub(crate) fn get_parsed_command_line_of_config_file_with_stack(
    ...
    let resolved_path =
        tsox_core::tspath::get_normalized_absolute_path(config_file_name, current_dir);
    ...
    let root: tsox_core::json::Value = if jsonc.trim().is_empty() {
        tsox_core::json::Value::Object(tsox_core::json::Map::new())
    } else {
        match tsox_core::json::from_str(&jsonc) {
            Ok(v) => v,
            Err(e) => {
                result
                    .errors
    ...
    if let Some(extends) = root_obj.get("extends") {
        ...
        if !extends_paths.is_empty() {
            ...
            for ext_path in &extends_paths {
                let ext_resolved =
                    tsox_core::tspath::get_normalized_absolute_path(ext_path, current_dir);
                let parent =
            ...
            let own_config_dir = tsox_core::tspath::get_directory_path(config_file_name);
            let compare_opts = tsox_core::tspath::ComparePathsOptions {
            ...
            for (ext_path, parent) in &extended_configs {
                let ext_dir = tsox_core::tspath::get_directory_path(ext_path);
                let relative_difference =
                    tsox_core::tspath::convert_to_relative_path(&ext_dir, &compare_opts);
                let rewrite = |spec: &str| -> String {
                    if starts_with_config_dir_template(spec)
                        || tsox_core::tspath::is_rooted_disk_path(spec)
                    {
                        spec.to_string()
                    } else {
                        tsox_core::tspath::combine_paths(&relative_difference, &[spec])
                    }
                };
    ...
    if let Some(references) = root_obj.get("references").and_then(|v| v.as_array()) {
        let config_dir_for_refs = tsox_core::tspath::get_directory_path(config_file_name);
        result.references = references
            .iter()
            .filter_map(|entry| {
                let raw_path = entry.as_object()?.get("path")?.as_str()?;
                Some(tsox_core::core::project_reference::ProjectReference {
                    path: tsox_core::tspath::get_normalized_absolute_path(
                        raw_path,
                        &config_dir_for_refs,
                    ),
    ...
    if let Some(co) = root_obj.get("compilerOptions").and_then(|v| v.as_object()) {
        result.raw_options = Some(tsox_core::json::Value::Object(co.clone()));
        ...
        let config_dir_for_opts = tsox_core::tspath::get_directory_path(config_file_name);
