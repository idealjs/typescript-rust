...
pub(crate) fn expand_file_names(
    ...
    let exclude_dirs: Vec<String> = effective_exclude
        ...
        .map(|p| tsox_core::tspath::get_normalized_absolute_path(p, base_dir))
        .collect();
    let exclude_globs: Vec<Glob> = effective_exclude
        .iter()
        .filter_map(|p| {
            let spec = if tsox_core::tspath::path_is_absolute(p) {
                p.clone()
            } else {
                tsox_core::tspath::combine_paths(base_dir, &[p])
            };
            Glob::parse(&spec).ok()
        })
        .collect();
    ...
    let add = |path: &str, out: &mut Vec<String>, seen: &mut std::collections::HashSet<String>| {
        let abs = tsox_core::tspath::get_normalized_absolute_path(path, base_dir);
...
pub(crate) fn match_glob_spec(spec: &str, base_dir: &str, fs: &dyn FS) -> Vec<String> {
    ...
    let abs_spec = if tsox_core::tspath::path_is_absolute(spec) {
        ...
        tsox_core::tspath::combine_paths(base_dir, &[spec])
    };
...
pub(crate) fn walk_and_collect_files(dir: &str, fs: &dyn FS, results: &mut Vec<String>) {
    ...
    for d in &entries.directories {
        let full = tsox_core::tspath::combine_paths(dir, &[d]);
