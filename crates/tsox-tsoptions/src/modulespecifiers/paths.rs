...
pub fn try_get_real_file_name_for_non_js_declaration_file_name(file_name: &str) -> String {
    ...
    let no_extension = tsox_core::tspath::remove_extension(file_name, ".ts");
...
pub fn get_js_extension_for_declaration_file_extension(ext: &str) -> String {
    match ext {
        ...
        tsox_core::tspath::EXTENSION_DCTS => tsox_core::tspath::EXTENSION_CJS.to_string(),
        _ => {
            let start = ".d".len();
            let end = ext
                .len()
                .saturating_sub(tsox_core::tspath::EXTENSION_TS.len());
            if start <= end {
...
pub fn get_relative_path_if_in_same_volume(
    ...
    if tsox_core::tspath::is_rooted_disk_path(&relative_path) {
        return String::new();
    }
    relative_path
}
