...
use crate::ast::{
    ModifierFlags, Node, NodeData, SymbolFlags, SyntaxKind,
};
...
impl Checker {
    ...
    pub(crate) fn global_interface_has_property(&mut self, symbol_name: &str, prop_name: &str) -> bool {
    ...
    pub(crate) fn collect_global_interface_member_names(&self, interface_name: &str) -> Vec<String> {
        ...
        if let Some(structured) = obj_type.as_structured() {
            if let Some(member_symbol) = structured.members.get(name_text) {
                ...
                if let Some(declaring_class) = self.declaring_class_of_member(member_symbol) {
                    ...
                    let is_private = crate::checker::exports::
    ...
    pub(crate) fn global_constructor_value_has_property(&mut self, obj_expr: &Arc<Node>, name: &str) -> bool {
        if obj_expr.kind != SyntaxKind::Identifier {
            return false;
        }
    ...
    #[allow(dead_code)]
    pub(crate) fn property_exists_on_non_nullable_part(&mut self, t: &Arc<Type>, name: &str) -> bool {
