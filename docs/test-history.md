...
> 本文件是**按时间正序追加的原始运行记录**（数据存档，非叙述文档）。
> 方法论与当前状态见根目录 [`TESTING.md`](../TESTING.md)。

# 当前批次
## Page-4 轮续（页 15-，compiler#1501 起）：页15/16/20/22-30 一次过；页31 修 import-equals 失败解析的 2438/2440 门控（importEqualsError45874）；页32 一次过（31P/15S/54ad）；页33 一次过（55P/2S/43ad）；页34 一次过（49P/7S/44ad，instanceofOperator 含非 UTF-8 字节按设计 skip）；页35 一次过（53P/14S/33ad）；页36 一次过（39P/19S/42ad）；页37 一次过（2P/98S/0ad，jsFile 族 allowJs/outFile 窗口）；页38 一次过（18P/33S/49ad，jsxEmptyExpressionNotCountedAsChild 超时 SKIP 已单跑核实 23s PASS——多 jsx 配置矩阵并发挤预算，同 emitHelpers 族）；页39 起点 2 FAIL→修复→终态 40P/22S/38ad/**0F**；页40 一次过（47P/5S/48ad）；页41 一次过（44P/6S/50ad）；页42 一次过（40P/19S/41ad）；页43 起点 1 FAIL→修复（switch break 出口累积器化，#55986）→终态 58P/7S/35ad/**0F**；页44 起点 2 FAIL＋1 signal 6 崩溃→三修→终态 66P/2S/32ad/**0F**；页45 一次过（44P/22S/34ad，noImplicitReturns/noEmitHelpers 等 skip 窗）；页46 一次过（54P/13S/33ad）；页47 一次过（39P/4S/57ad，numericUnderscoredSeparator 超时 SKIP 已单跑核实 13s PASS——8 target 配置矩阵并发挤预算，同族）；页48 一次过（49P/14S/37ad）；页49 一次过（30P/42S/28ad，pathMapping 族 baseUrl 不支持密集窗）；页50 一次过（51P/11S/38ad）；页51 起点 1 signal 6 崩溃→修复（延迟条件默认约束回退深度上限 100）→终态 35P/14S/51ad/**0F**；页52 起点 1 FAIL＋1 panic→修 panic（正则旗标扫描按完整 UTF-8 字符推进）＋登记 reexportDefaultIsCallable 日期组（默认再导出链 TYPE 意义缺口）→终态 60P/3S/37ad/**0F**；页53 一次过（35P/39S/26ad，requireOfJson 族 resolveJsonModule 不支持密集窗）；页54 一次过（49P/7S/44ad）；页55 一次过（72P/9S/19ad，sourceMap 窗口）；页56 一次过（52P/3S/45ad）；页57 起点 2 FAIL→修复（嵌套类计算属性名中的 super 锚定外层类）→终态 48P/5S/47ad/**0F**；页58 一次过（28P/38S/34ad，systemModule 族 module=System 不支持密集窗）；页59 一次过（44P/10S/46ad）；页60 一次过（49P/3S/48ad）；页61 一次过（49P/6S/45ad）；页62 一次过（43P/12S/45ad）；页63 一次过（2P/97S/1ad，using/variance 族不支持选项密集窗）；页64 一次过（29P/54S/17ad，noUnusedLocals/Parameters 密集窗）；页65 一次过（23P/2S/12ad，尾页 37 条）——**compiler 套件 6,537 条全部完成，全 0 FAIL**；

# 全量复跑轮（2026-09-03，6 并发）

## Sweep #1（修复轮）：12,444 条 3 FAIL → 三修一登记（提交 89da27f）→ **Sweep #2（验证轮）：12,444 条全 0 FAIL**（5,664P/2,306S/4,474ad）+ transpile 22 条 0 FAIL = 12,466 全绿；四门 1353/2/1010/15 绿

- **neverAsDiscriminantType（compiler#4401-4500，strict=false）**：`get_optional_type` 的 `| undefined` 只在 strictNullChecks 下加（对齐 Go addOptionality）——非严格下 `{kind?: never}` 保留 never 判别（此前被并成 undefined，绕过 never 双臂剔除又被「非严格 undefined 可赋给一切」保留 → TS2339 假阳）；两条探针行实为 strict=true/false 两配置、fwd 结果差异即非严格赋值规则所致。
- **classInConvertedLoopES5（compiler#701-800，回归）**：mapped 索引访问模板重解守卫——`{[P in K]: T}[string]` 重解只带 {P→index} 映射，Record 的别名实参（T:=object）只存在于急切代入的 template_type；模板节点（按名）不引用映射参数时直接用 template_type，否则裸 `T` 泄漏（Page-3 轮后回归窗口内暴露；keyRemappingKeyofResult2 复跑 PASS 无回归）。
- **arrayToLocaleStringES2020（compiler#201-300，Page-2 轮起携带）**：登记日期组——裸 Array 惰性成员表 vs 实例化 ReadonlyArray 成员的泛型签名比较：es2019 `flatMap<U, This>` 经 instantiate_signature_in_context_of 规范化，嵌套 callback 返回位的 U 推理不落地（手写接口同形可映射、Array 惰性成员不行）；es2015 无 flatMap 故过、es2020 累积加载后败。另附修：签名推理 callback 返回位按需解析（inference.rs infer_from_signature 用解析 getter 而非惰性 OnceLock）。es2015 兄弟全绿。
- Sweep #2 的 5 例超时 SKIP 全部单跑核实（90s 预算）：emitHelpersWithLocalCollisions 20s PASS / arbitraryModuleNamespaceIdentifiers_module 21s PASS / callChainWithSuper 26s PASS / tsxReactEmitSpreadAttribute 24s PASS / jsxRuntimePragma 30s 台账 DIFF——均为多配置矩阵并发挤 30s 预算族，非挂死。

# conformance 轮（页 0- 起）
## conformance 页 0（#1-100）一次过：15P/62S/23ad/**0F**（asyncAwait es5/es2017 emit 窗口 noEmitHelpers 密集）；页1（#101-200）一次过：3P/88S/9ad/**0F**（asyncAwait es5/es6 emit 窗口）；页2（#201-300）一次过：33P/19S/48ad/**0F**（classAbstract/classExtends 窗口）；页3（#301-400）一次过：35P/7S/58ad/**0F**（classStaticBlock/constructorAccessibility/privateProtected 窗口）；页4（#401-500）一次过：36P/17S/47ad/**0F**（derivedClass/privateName 窗口）；页5（#501-600）一次过：38P/19S/43ad/**0F**（privateName 全家/mixin 窗口）；页6（#601-700）一次过：30P/31S/39ad/**0F**（accessorsOverride/autoAccessor/propertyOverrides/staticIndexSignature/constEnum/controlFlow 窗口）；页7（#701-800）一次过：55P/12S/33ad/**0F**（controlFlow 全家/decorator 窗口）；页8（#801-900）一次过：32P/17S/51ad/**0F**（decoratorOnClass/importCall 窗口）；页9（#901-1000）一次过：33P/22S/45ad/**0F**（importCall 续/emitter/enum/globalThis 窗口）；页10（#1001-1100）一次过：57P/2S/41ad/**0F**（Intl/logicalAssignment/symbol 窗口，arbitraryModuleNamespaceIdentifiers_module 超时 SKIP 已单跑核实 21s PASS——12 module 配置矩阵并发挤预算，同族）；页11（#1101-1200）起点 1 FAIL→修复（TS2692 原始-包装器 elaboration 移植）→终态 44P/24S/32ad/**0F**
- **conformance 页11**：**TS2692 移植**（Go tryElaborateErrorsForPrimitivesAndObjects）——全局 String/Number/Boolean/Symbol 包装器接口对同名原始类型赋值失败时，在泛化 head 之前压入 2692 elaboration（渲染为 TS2322 下的嵌套行；symbolType15 的 `symObj: Symbol` 赋给 `symbol`）。
- **conformance 页12（#1201-1300）一次过**：64P/0S/36ad/**0F**（computedPropertyNames 全家/superCallBeforeThis 窗口）；页13（#1301-1400）一次过：63P/3S/34ad/**0F**（computedPropertyNames 续/decorator.es6/destructuring 窗口）；页14（#1401-1500）一次过：53P/11S/36ad/**0F**（destructuring 续/iterableArrayPattern 窗口）；页15（#1501-1600）一次过：44P/4S/52ad/**0F**（iterableArrayPattern 续/for-of 全家/FunctionDeclaration_es6 窗口）；页16（#1601-1700）一次过：48P/19S/33ad/**0F**（MemberFunctionDeclaration/exportsAndImports/multipleDefaultExports 窗口）；页17（#1701-1800）起点 3 FAIL→修复→终态 48P/2S/50ad/**0F**（templateString 全家窗口；2 skip 全为不支持选项 downleveliteration/usedefineforclassfields）。
- **conformance 页17**：**TS2349/2351 嵌套行移植**（Go invocationErrorDetails, checker.go ~L10115）——head "This expression is not callable/constructable" 从不带裸输出，必带嵌套链行：原始类型 callee 的 apparent type 是全局包装器接口（`string`→`String`→"  Type 'String' has no call signatures."，templateStringInCallExpression(.es6)/templateStringInNewExpression 3 例）；联合 callee 为 No/Not-all-constituents-of-type 变体（Not-all 再嵌一层首个无签名成员行）。实现为 `report_invocation_error` + `primitive_apparent_name`（四站点统一收口：联合构造臂/联合调用臂/非结构化原始/结构化无签名）。
- **conformance 页18（#1801-1900）一次过**：77P/0S/23ad/**0F**；页19（#1901-2000）一次过：74P/0S/26ad/**0F**；页20（#2001-2100）一次过：46P/4S/50ad/**0F**（skip 全为选项门）；页21（#2101-2200）一次过：24P/66S/10ad/**0F**（66 skip 全为 noemithelpers/moduleResolution=Classic 选项门）；页22（#2201-2300）一次过：37P/32S/31ad/**0F**。
- **conformance 页23（#2301-2400）起点 1 FAIL→修复→终态 29P/13S/58ad/**0F**：**联合目标失败 elaboration 移植**（Go typeRelatedToSomeType 报告尾 + getBestMatchingType 子集，relater.go ~L3045/~L872）——整联合失败后按最优匹配成分重跑对比并压入中间层头行（functionExpressionContextualTyping2：`S`→`A|B` 失败得三层 head/S-vs-A/boolean-vs-number）；成分挑选策略落地 Reference 同 target/alias 同符号、对象字面量避数组、invokable 签名偏好三族（discriminant/index-overlap 未移植，None 仅少一行不误报）。
- **conformance 页24（#2401-2500）起点 1 FAIL→修复→终态 48P/5S/47ad/**0F**：**联合源失败 elaboration 移植**（Go eachTypeRelatedToType 报告语义，tsgo 二进制探针核实规则）——失败 nullish 成分优先占叶行（`number|undefined`→boolean 拼 undefined）；无 nullish 失败时首败成分重跑出深链行+泛化头行（`string|number`→string 拼 number；字面量联合两层都泛化 `1|2`→number）；缺属性链顶匹配时成员头抑制（`{x:1}|{y:2}`→`{x:1;y:2}` 只留缺属行）。typeGuardInClass。
- **conformance 页18-22、25-33、35、37-42 均一次过 0F**（页18:77P/0S/23ad；页19:74P/0S/26ad；页20:46P/4S/50ad；页21:24P/66S/10ad——66 skip 全为 noemithelpers/Classic 选项门；页22:37P/32S/31ad；页25:29P/7S/64ad；页26:58P/22S/20ad；页27:35P/20S/45ad；页28:36P/18S/46ad；页29:42P/2S/56ad；页30:54P/45S/1ad；页31:2P/98S/0ad——98 skip 全为 allowJs/resolvejsonmodule 选项门；页32:4P/92S/4ad；页33:14P/78S/8ad；页35:24P/17S/59ad；页37:44P/22S/34ad；页38:59P/10S/31ad；页39:60P/0S/40ad；页40:45P/2S/53ad；页41:65P/0S/35ad；页42:81P/0S/19ad）。
- **conformance 页34（#3401-3500）起点 4 FAIL→修复→终态 32P/3S/65ad/**0F**：**JSX 每文件 pragma**——`/** @jsx dom */` 领头多行注释（GetLeadingCommentRanges 扫描，行内首 @ 规则、末次生效）覆盖 jsxFactory 选项参与 TS2874 门（Go getLocalJsxNamespace；inlineJsxFactory* 3 例）；**TS2875 锚点**改最外层 JSX 表达式（含 fragment，jsxJsxsCjsTransformCustomImport (2,11)）。
- **conformance 页36（#3601-3700）起点 1 FAIL→修复→终态 23P/41S/36ad/**0F**：**node10 alternate-result 家族**——①TS5107 弃用告警（显式 moduleResolution=node10 时无文件诊断+aka.ms/ts6 嵌套行；Go 二进制实测 node10 已移除报 5108，基线为 TS5.x 语义照基线实现）②node10 裸说明符解析失败后以 Exports|Imports|SelfName 全特性重探，命中则填 ResolvedModule.alternate_result③TS2307 挂 6280 "There are types at" 嵌套行④**harness render_errors_baseline 补 !!! 全局错误段**（Go GetErrorBaseline 无定位块；540 份带 !!! 参考基线此前 0 通过、全在台账/跳过内，改动安全）。另页35/37 共 6 例超时 SKIP 全部单跑核实 PASS（8-24s，多 module 配置矩阵并发挤 30s 预算族）。
- **conformance 页43-50、53-57 均一次过 0F**（页43:49P/6S/45ad；页44:51P/1S/48ad；页45:59P/0S/41ad；页46:28P/67S/5ad——61 allowJs 选项门；页47:7P/86S/7ad——86 全为选项门 9s 完赛；页48:30P/19S/51ad；页49:23P/44S/33ad；页50:29P/17S/54ad；页53:46P/1S/53ad；页54:47P/2S/51ad；页55:37P/2S/61ad；页56:31P/2S/67ad；页57:59P/0S/41ad）。
- **conformance 页51（#5101-5200）起点 1 FAIL→登记日期根因组→终态 31P/3S/66ad/**0F**：**字符串映射（Uppercase/Lowercase/Capitalize/Uncapitalize）子系统未移植**（templateLiteralTypes1 四缺口同根：`Capitalize<T>` 落 error_type 显 `get${error}`（StringMapping 无生产者、relater 三入口皆 stub）；同态映射恒等 `T`→`{[P in keyof T]: T[P]}` 假阳；`{[P in B as \`p_${P}\`]: T}` 模板键名映射比较+TP 链缺；TS2590 锚点差。同族 templateLiteralTypes2-8/Patterns 刷新轮已在案）。
- **conformance 页52（#5201-5300）起点 2 FAIL→双修→终态 46P/3S/51ad/**0F**：①**never-交集 callee**——TS2349 联合臂收窄为纯 Union 旗标（Go invocationErrorDetails flags 检查），交集判 never-交集（同名字段跨原始域互斥/同域异字面量→never，Go getReducedType IsNeverIntersection 近似）显 "Type 'never' has no call signatures."（neverIntersectionNotCallable）②**TP 实例化注记对 IndexedAccess 目标生效**——Go reportRelationError 的 targetFlags 视图（目标 IndexedAccess 且源非时取对象类型旗标），`T[P]`（T 为类型参数）挂 "'T[P]' could be instantiated..." 嵌套行（nonPrimitiveConstraintOfIndexAccessType）。
- **conformance 页58（#5801-5907，终页 107 条）起点 1 FAIL→修复→终态 34P/7S/66ad/**0F**：**等性判别窄化的双臂剔除守卫改 never-only**（Go narrowTypeByDiscriminant 仅查 discriminantType.flags&Never）——`err === undefined` 须保留 `err: undefined` 成员（discriminatedUnionTypes3 #44435 假阳 never→TS2339）；narrowingByDiscriminantInLoop/narrowingUnionWithBang 复跑 PASS 无回归。
- **transpile 套件 22 条**：22 accepted-diff（台账在案）、0 FAIL、0S——**三大套件 12,466 条全量跑毕：compiler 6,537 + conformance 5,907 + transpile 22，全 0 FAIL**（2026-09-03，Page-4 轮收官）。
页17 修推断爆炸；页18 修多级约束链；页19 三连修＋清台账 1 组；页21 修 private 同声明；各页终态均 **0 FAIL**（页23：49P/15S/36ad；页24：46P/18S/36ad；页25：30P/32S/38ad；页26：47P/9S/44ad；页27：78P/2S/20ad；页28：58P/7S/35ad；页29：51P/4S/45ad；页30：52P/3S/45ad；页31：38P/23S/39ad——4 例 importHelpers×esmoduleinterop 10 配置矩阵并发挤爆 30s 预算计超时 SKIP，首轮串行 22-30s 已 PASS，与页21 emitHelpers 同族；页33 起），四套门全绿
...
- **页44 三连修**：①**布尔字面量恒假判定读 literal_value**——布尔字面量是 TypeData::Literal（intrinsic_name 恒 None）→ `true` 也被判 definitely-falsy → `if (x.kind)` 对 `{kind:true}|{kind:false}` 两个成员都保留/删除（narrowingByDiscriminantInLoop #9977）。②**判别属性为 never/undefined-only 的成员双臂剔除**（Go narrowTypeByDiscriminant 的 filter 无条件 `discriminantType.flags&Never==0`；非 SNC 下 `{kind?: never}` 表示为 undefined-only 需一并覆盖——neverAsDiscriminantType）。③**is_matching_reference 解包 NonNullExpression 目标**——skip_parentheses 只剥括号，`t.thing!` 作窄化引用时自递归爆栈（signal 6，存量崩溃）；并把判别窄化泛化到 Node 子引用（`t.thing!.name !== "C"` 窄化内层联合，可选链准入，引用侧先解包 paren/non-null）——narrowingUnionWithBang。
- **页39 双修**：①**mapped 类型索引访问按 P:=index 实例化模板**——原直接返回原始模板（自由映射参数 K 残留，`Values<{[K in keyof T as K & string]: {src: K;…}}>` 显示/约束检查都拿 `{src: K}`）；改为域判定通过后按 type_argument_stack 推入 P→index 重解模板节点（keyRemappingKeyofResult2，#56239）。②**流深度截断返回 error 类型**（Go flow.go ~L122 返回 errorType 而非查询种子 `T|undefined`）——TS2563 触发后同一走查不再把 undefined 交给 2454 检查（largeControlFlowGraph 幻影 2454）。jsxRuntimePragma（5 jsx 配置 30s 临界）串行 accepted-diff 已核实。
